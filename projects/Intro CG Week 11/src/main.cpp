...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		float     lightAmbientPow = 100.5f;
		float     lightSpecularPow = 50.0f;
		...
		shader->SetUniform("u_everything", everything);
		...
		imGuiCallbacks.push_back([&]() {
			...
			//Added button toggles for each effect in the ImGUI
			if (ImGui::CollapsingHeader("A1 Debug Toggles"))
			{
				if (ImGui::Button("No Lighting"))
				{
					...
					shader->SetUniform("u_everything", everything = 0);
				}
				...
				if (ImGui::Button("Everything"))
				{
					...
					shader->SetUniform("u_specularOnly", specularOnly = 0);
					shader->SetUniform("u_ambientSpecular", ambientSpecular = 0);
					shader->SetUniform("u_everything", everything = 1);
				}
			}
		...
		Texture2D::sptr penguinTexture = Texture2D::LoadFromFile("images/penguin_texture.png");
		Texture2D::sptr waterTexture = Texture2D::LoadFromFile("images/water_texture.png");
		Texture2D::sptr snowTexture = Texture2D::LoadFromFile("images/snowtexture.jpg");
		...
		material2->Set("s_Diffuse", penguinTexture);
		...
		material3->Set("s_Diffuse", waterTexture);
		...
		material4->Set("s_Diffuse", snowTexture);
		...
		//Changed Obj to plane. Added a 90 degree rotation to the plane
		GameObject sceneObj = scene->CreateEntity("scene_geo"); 
		{
			VertexArrayObject::sptr sceneVao = ObjLoader::LoadFromFile("models/water.obj");
			sceneObj.emplace<RendererComponent>().SetMesh(sceneVao).SetMaterial(material3);
			sceneObj.get<Transform>().SetLocalPosition(0.0f, 0.0f, 0.0f);
			sceneObj.get<Transform>().SetLocalRotation(90.0f, 0.0f, 0.0f);
			sceneObj.get<Transform>().SetLocalScale(3, 3, 3);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(sceneObj);
		}

		GameObject obj1 = scene->CreateEntity("bigGlacier");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/bigglacier.obj");
			obj1.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material4);
			obj1.get<Transform>().SetLocalPosition(0.0f, -5.0f, 1.8f);
			obj1.get<Transform>().SetLocalRotation(90.0f, 0.0f, 0.0f);
			obj1.get<Transform>().SetLocalScale(2, 3, 5);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj1);
		}

		GameObject obj2 = scene->CreateEntity("penguin1");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/penguin.obj");
			obj2.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material2);
			obj2.get<Transform>().SetLocalPosition(50.0f, 20.0f, 1.0f);
			obj2.get<Transform>().SetLocalRotation(90.0f, 0.0f, 0.0f);
			obj2.get<Transform>().SetLocalScale(1,1,1);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj2);
		}

		GameObject obj3 = scene->CreateEntity("penguin2");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/penguin.obj");
			obj3.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material2);
			obj3.get<Transform>().SetLocalPosition(50.0f, 100.0f, 1.8f);
			obj3.get<Transform>().SetLocalRotation(90.0f, 0.0f, 0.0f);
			obj3.get<Transform>().SetLocalScale(1, 1, 1);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj3);
		}

		GameObject obj4 = scene->CreateEntity("smallGlacier");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/smallglacier.obj");
			obj4.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material4);
			...
			pathing->Points.push_back({  10.0f,  0.0f, 0.0f });
			pathing->Points.push_back({   0.0f, 10.0f, 0.0f });
			pathing->Points.push_back({   0.0f,  0.0f, 0.0f });
			pathing->Speed = 5.0f;
		}

		GameObject obj5 = scene->CreateEntity("penguin3");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/penguin.obj");
			obj5.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material2);
			obj5.get<Transform>().SetLocalPosition(50.0f, 50.0f, 1.8f);
			obj5.get<Transform>().SetLocalRotation(90.0f, 0.0f, 0.0f);
			obj5.get<Transform>().SetLocalScale(1, 1, 1);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj3);
		}

		GameObject obj6 = scene->CreateEntity("penguin4");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/penguin.obj");
			obj6.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material2);
			obj6.get<Transform>().SetLocalPosition(30.0f, 40.0f, 0.7f);
			obj6.get<Transform>().SetLocalRotation(90.0f, 0.0f, 0.0f);
			obj6.get<Transform>().SetLocalScale(1, 1, 1);
